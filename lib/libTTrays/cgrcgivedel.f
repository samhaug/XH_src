c -----------------------------------------------------------------
      subroutine cgrcgivedel(ths,phs,thr,phr,del,ndel,MXP,pnts,idb,epd,imima)
...
c    transform to xyz coord.
      call dcxyz(ths,phs,xs,ys,zs)
      call dcxyz(thr,phr,xr,yr,zr)
...
c    calc. vector perpend. to xs in greatc plane
      ...
      rl=1.d0/(sqrt(xd*xd+yd*yd+zd*zd))
