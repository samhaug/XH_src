     ...
     1             'pP','pPP','sS','sSS'/
      ...
      ierr=0
      if(hdep.gt.725) then
c      write(6,*) 'dchkdiff: event depth > 725'
       ierr=1
       return
      endif
...
c -------------------------
      if(lphin.eq.3.and.phin(1:3).eq.'ScS') then
       ...
       lout=istlen(phout)
c      if(lphin.ne.lout) write(6,*) phin(1:lphin),' identified as ',phout(1:lout)
       return
      endif
...
c -----------
      if(lphin.eq.2.and.phin(1:2).eq.'sS') then
       call splint(delmxsdep,delmxsdel,delmxsspl,ndelmxs,hdep,delcrith)
       call splint(delmxsdep,delmxsdel,delmxsspl,ndelmxs,0.,delcrit0)
c      add distance for critical ray corresponding to up going part
       ddel=delcrit0-delcrith
       delcritss=delcrit0+ddel
c      write(6,*) 'maximum delta for direct wave for source at this depth: ',delcritss
...
c -------------------
      if(phin(1:2).eq.'pP') then
       call splint(delmxpdep,delmxpdel,delmxpspl,ndelmxp,hdep,delcrith)
       call splint(delmxpdep,delmxpdel,delmxpspl,ndelmxp,0.,delcrit0)
c      add distance for critical ray corresponding to up going part
       ddel=delcrit0-delcrith
