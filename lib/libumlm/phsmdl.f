      subroutine phsmdl(lumes,cpert,ifnorm)
...
c
      ...
      cou=amp(1)*6371./(sqrt(float(lord*(lord+1)))*amp(5))
      nnker=5
    ...
    7 format('maximum l of model =',i4)
      ...
      do 60 im=1,numm
      fac=1.
      if(im.eq.1.and.ifnorm.eq.1) fac=rt2
      k=k+1
      iadd=ind+im
      cpert(k)=0.
      do 70 iker=1,nnker
   70 cpert(k)=cpert(k)+fac*pertm(iadd+leny*(iker+1))*aker(iker)
      cpert(k)=cpert(k)+fac*pertm(iadd)*amp(7)+pertm(iadd+leny)*amp(8)
   60 cpert(k)=cpert(k)*cou*100./amp(1)
   50 continue
      return
      end
