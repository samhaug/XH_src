...
float sawspl(ord,nknots,knot,xi)
int ord,nknots;
float *knot,xi;
/* ord: number of rho(x)
...
/* printf("sawspl: ord = %d   nknots = %d\n",ord,nknots);
 if(ord==0){
   for (ii=0 ; ii<nknots ; ii++ ){ 
     printf("knot %f",knot[ii]);
     if(ii < nknots-1){printf("  %f",knot[ii+1]-knot[ii]);}
     printf("\n");
   }
   } */
 ...
 if (nknots!=1) {
  ...
  free_farray1(hh,0);
	     ...
	     else  if (xi<0.9*knot[0])
		 {
		     rho_x=0;
		 }
	     else
		 {
		     rho_x=0.5*(1.0+cos(4*atan(1.)*(xi-knot[0])/(0.1*knot[0])));
		 }
	 }
/* printf("arg = %f   value = %f\n",xi,rho_x) */ ;
  return(rho_x);  
}

void sawfill_hh(hh,knot,Nx)
float *hh,*knot;
int Nx;
{
int ii;
...
float d_sawspl(ord,nknots,knot,xi)
int ord,nknots;
float *knot,xi;
/* ord: order of rho(x)
   nknots : # of knkots = Nx+1 (Nx=index of highest spline)
...
float coefa,coefb,coefc;
  ...
  /* Consistency checks */
  if((xi-(float)TOL)>knot[Nx])
    stop("d_sawspl: xi>knot[Nx]");
  else if((xi+(float)TOL)<knot[0])
    stop("d_sawspl: xi<knot[0]");
  else if(ord>Nx)
    stop("sawspl: order > Nx");
  ...
  float denom,denomsum,dd;
    ...
    if(xi>=knot[ord-1]&&xi<=knot[ord]){			/* x0<=x<=x1 */
      ...
      rho_x=3.*coefa*(xi-knot[ord+1])*(xi-knot[ord+1])*(xi-knot[ord+1]);
      rho_x+=2.*coefb*(xi-knot[ord+1]);
      rho_x+=coefc;
    }
    else						/* x>x3 */
...
float d2_sawspl(ord,nknots,knot,xi)
int ord,nknots;
float *knot,xi;
/* ord: order of rho(x)
   nknots : # of knkots = Nx+1 (Nx=index of highest spline)
...
float coefa,coefb;
  ...
  /* Consistency checks */
  if((xi-(float)TOL)>knot[Nx])
    stop("d2_sawspl: xi>knot[Nx]");
  else if((xi+(float)TOL)<knot[0])
    stop("d2_sawspl: xi<knot[0]");
  else if(ord>Nx)
    stop("d2_sawspl: order > Nx");
...
float sawspl_(pord,pnknots,knot,pxi)
     int *pord, *pnknots;
     float *knot, *pxi;
{
  return(sawspl(*pord,*pnknots,knot,*pxi));
    }
