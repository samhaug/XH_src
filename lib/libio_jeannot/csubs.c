#include <sys/types.h>
#if ( defined(Machine4) | defined(Machine5) | defined(MachineA) )
#define access rmtaccess        /* remove this line for no remote access */
#endif
...
//	  {for (lm=0, mp=mess, cp=strerror[*ierrno];*cp;mp++, cp++)
	     { *mp = *cp; lm++; } 
	   *lmess=(int)lm;
	  }
...
/*	extern int      open(); */
	...
	errno = 0;
	*ichan = (int) open(F_STRP(pname), kopt, (int)(*mode) );
	*ierrno = (int) errno;
}
...
#if ( defined(Machine3) | defined(Machine4) | defined(Machine5) | defined(MachineA) )
...
#endif
#if ( defined(Machine4) | defined(Machine5) | defined(MachineT) | defined(MachineA) )
