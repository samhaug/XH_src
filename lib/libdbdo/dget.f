...
C     equivalence (ibig,rbig)
      character*200 line
      character*100 grami,reqi

      parameter(MXDFVOL=15)
      character*80 dfvols,sitel,fgetenv,seedv
...
C     character*3 site
      ...
      sitel=fgetenv('SITE_LIST',lsitel)
      thsite=fgetenv('THIS_SITE',lthsite)
      ...
      do while(ips.lt.lsitel)
        ...
        open(lusite(nsite),file=reqi(1:lreq)//':'//site(nsite))
        iwsite(nsite)=0
  ...
  122     continue
     ...
     1                //' '''//seedv(1:lseedv)//''' '''//sname(1:lsname)//''''
      ...
      do isite=1,nsite
        ...
        if(iwsite(isite).eq.0) then
          call cunlink(reqi(1:lreq)//':'//site(isite)//'\0',ires,ierrno)
        endif
      enddo

      if(iflocl.ne.0.and.iwsite(1).ne.0) then
        call ldget(reqi(1:lreq)//':'//site(ithiss),gflag,gram,ifstat,iflap,ierr)
      else
        do isite=1,nsite-1
          if(iwsite(isite).ne.0) then
             call cunlink(grami(1:lgram)//':'//site(isite)//'\0',ires,ierrno)
             call cunlink(grami(1:lgram)//':'//site(isite)//'.Z\0',ires,ierrno)
             write(6,'(a)') site(isite)//'.dget '
     1                      //reqi(1:lreq)//':'//site(isite)//' '
     1                      //grami(1:lgram)//':'//site(isite)
     1                      //' < /dev/null'
     ...
     1                      //' < /dev/null '''  )
        ...
        do isite=1,nsite-1
          if(iwsite(isite).ne.0) then
             isys=system(  'csh -c ''( await '//grami(1:lgram)//':'//site(isite)//' ) < /dev/null'' ' )
             if(ifs.ne.2) then
               call cpgram(grami(1:lgram)//':'//site(isite),grami(1:lgram),ifs,iflap,ierr)
               if(ierr.ne.0) then
               ...
               isys=system(  '( rm -f '//grami(1:lgram)//' ; mv '
     1     //grami(1:lgram)//':'//site(isite)//' '//grami(1:lgram)
     1     //' ) < /dev/null' )
             endif
             ifs=3
          endif
