...
c
     ...
     1   (llp*rkl2+nnp*rkn2+rhop*rkr2)*hnh
      temp(6)=temp(6)+add
cccc
cccc  p velocity in the lithosphere
cccc
ccc     if(iq.le.moho.or.iq.ge.n220) then
ccc        if(jcom.ne.2) then
ccc          add=wt(il)*rrho*delv2*(rka0+rkc0+rkf0)*hnh
ccc          temp(8)=temp(8)+add
ccc        endif
ccc      endif
c
c    inner core vs
c
      if(iq.lt.nic.and.jcom.ne.2) then
        vs=sqrt(ll/rrho)
        vvp=sqrt(aa/rrho)
        ddrho=0
        ddvs2=2.*vs*delv
...
c
      anorm=acum(13)*omn2
c
c     if(jcom.eq.2) write(6,1) nord,lord,wcom,anorm,parm(12)
c    1   ,parm(6)
c   1 format(1x,i2,' t',i3,' wcom:',e12.5,' anorm:',e12.5,
c    1  5h 'b':,e10.4,' alpha:',e10.4)
c     if(jcom.ne.2) write(6,2) nord,lord,wcom,anorm,parm(12)
c    1   ,parm(6)
c   2 format(1x,i2,' s',i3,' wcom:',e12.5,' anorm:',e12.5,
c    1  5h 'b':,e10.4,' alpha:',e10.4)
c     avtest=-u(nsl)*wcom**2
c     ahtest=-v(nsl)*wcom**2
c     write(6,111) avtest,parm(3),ahtest,parm(4)
c 111 format(' av',2e12.5,'  ah',2e12.5)
      return
      end
